...
db = pymysql.connect(host='101.235.203.94', port=3105, user='sis_user', passwd='sis_user1!', db='SIS_EMPLOYMENT',
...
def saramin_crawler(url):
    ...
    for page_number in range(1, page_count_maximum):
        ...
        print("- ", page_count_maximum, "/", page_number, " 페이지 종료")
    print("> 병합 완료!")
    ...
    try:
        ...
        sql = """INSERT INTO CPN_00 (COMPANY_NM, COMPANY_RNM, COMPANY_EXP_EDU, COMPANY_EMP_TYPE, COMPANY_STA_END) VALUES (%s, %s, %s, %s, %s)"""
        ...
        for detail_data in all_data:
            ...
            count = count + 1
            print(count, " 번째 데이터 입력 중")
            cursor.execute(sql, (find_company_nm, find_title, find_recruit_condition, find_company_info, find_support_info))
            print("완료! 회사 명 : ", find_company_nm)
